...
namespace LHAPDF {
  ...
  PDFInfo* mkPDFInfo(const string& setname_nmem) {
    const pair<string,int> idpair = lookupPDF(setname_nmem);
    return mkPDFInfo(idpair.first, idpair.second);
  }



  PDF* mkPDF(const string& setname, size_t member) {
    // Find the member data file and ensure that it exists
    const string searchpath = findpdfmempath(setname, member);
    if (searchpath.empty()) {
      const size_t setsize = getPDFSet(setname).size();
      if (member > setsize-1)
  ...
  AlphaS* mkAlphaS(const string& setname_nmem) {
    if (contains(setname_nmem, "/")) {
      const pair<string,int> idpair = lookupPDF(setname_nmem);
      return mkAlphaS(idpair.first, idpair.second);
    } else {
      return mkAlphaS(getPDFSet(setname_nmem));
    }
  }
