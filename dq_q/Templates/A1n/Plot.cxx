// Make a plot of the world A1n data 
...
int Plot(){
   ...
   TCanvas *c1 = new TCanvas("c1","A1n Data",1000,800); 
   c1->cd();
...
TGraphErrors *GetTGraphErrors(std::string exp,CSVManager *data){
   ...
   for(int i=0;i<N;i++){
      ...
      ey.push_back(arg); 
      if(gIsDebug) std::cout << Form("x = %.3lf, A1n = %.3lf ± %.3lf ± %.3lf (%.3lf)",x[i],y[i],stat[i],syst[i],ey[i]) << std::endl;
   }
